...
> Last updated on May 18 2023. Is any of this out of date? Please [yell at me on Twitter](https://twitter.com/jacobmparis) and I will fix it.
...
|                        | Vercel              | Fastly | Netlify | Cloudflare Pages | AWS Lambda |
| ---------------------- | ------------------- | ------ | ------- | ---------------- | ---------- |
| **Runtime**            | Node/V8 (per route) | WASM   | Node/V8 | V8               | Node       |
| **Bundle size**        | 50MB                | 20MB   | 50MB    | 5MB              | 50MB       |
| **Request body size**  | 4.5MB               | 8KB    | 3MB     | 100MB            | 6MB        |
| **Streaming**          | Yes                 | Yes    | No      | Yes              | Yes        |
| **Deferred data**      | Yes                 | Yes    | No      | Yes              | Yes        |
| **Server-sent events** | No                  | Yes    | No      | No               | Yes        |
...
- Fastly supports a 20MB bundle size limit.
...
- Netlify honors cache-control headers including `s-maxage` for their CDN, but does not support `stale-while-revalidate` or `stale-if-error`.
- The maximum request body size with Netlify is 3MB, which can be problematic for direct file uploads.
- Only cloud databases are supported
- Does not support streaming, so you can't use Server-Sent Events or `defer()`
- Does not support websockets

### Cloudflare Workers

Cloudflare Workers is being rolled into Cloudflare Pages, so I'd consider this section officially deprecated and you can find more details in the [Cloudflare Pages](#cloudflare-pages) section.

When you deploy a Remix app to Cloudflare workers, all of your pages will be compiled into a single worker script. Static files are also served from this worker, so you can't use Cloudflare's CDN for static assets.<SideNote> Pages does not have this limitation. </SideNote> and every request for a static asset (potentially dozens of requests for a single page) will count as a billable worker invokation.

That said, requests are very cheap so this may not be a deal-breaker for you.
...
- Pages only supports a bundle size of 5MB<SideNote> In comparison to Netlify's 50MB limit. </SideNote>.
