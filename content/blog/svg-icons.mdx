...
The best way is to use an svg spritesheet.
...
In this article, we will build a script that compiles a folder of svg icons into a single svg spritesheet. We will also build a React component that displays a specific icon by referencing its id, with full type safe autocomplete for the available icons.
...
If you run this script now, it will create a single `icon.svg` file in the `app/components` folder.
...
import spriteHref from "~/app/components/icon.svg"
...
## Create a JSON file with the list of icons
...
The JSON file will look like this. I would prefer to use an array here, but typescript will read the array as an array of strings and then we don't get the useful type names.
...
Import the JSON file and use the type of the `iconNames` property to type the `name` attribute of the `Icon` component.
...
## Generate the JSON file automatically

We can generate the JSON file automatically by reading the list of files in the `svg-icons` folder and using the filename as the key.

Go back to the `build-icons.ts` file and continue the script from right after the `generateSvgSprite` function.

We already have the list of files, so we can just map them into the format we want and use JSON.stringify() to convert it to JSON.
...
export const links: LinksFunction = () => {
  return [
    ...
    { rel: "preload", href: iconHref, as: "image", as="image/svg+xml"},
  ]
}
```
