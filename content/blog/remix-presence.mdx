...
Try out a [live example of what we're building](/examples/remix-presence)
...
export function usePresenceUsers(
  ...
  usePollInterval(() => {
    ...
    void fetch("/examples/remix-presence/presence", {
      method: "POST",
      credentials: "include",
      body,
    })
  ...
  return json(
    {
      ...
      initialUsers: Object.values(db.presences).filter(
        (user) =>
          user.lastSeenWhere === "/examples/remix-presence",
      ),
    },
    {
      headers: {
...
You can even navigate directly to it in your browser to see the stream in action at `http://localhost:3000/examples/remix-presence/presence` (or whatever your URL is).
...
Try out a [live example](/examples/remix-presence)
